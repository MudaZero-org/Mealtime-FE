...
const mealpackUtils = {
  ...
  renderInput: (meal, selectionArr, setSelectionArr) => {
    if (meal.selectedFav) {
			return (
				<input checked className="checkbox" type="checkbox" onChange={() => mealpackUtils.addToSelectedArr(meal)}></input>
			)
		} else {
			return (
				<input className="checkbox" type="checkbox" onChange={() => mealpackUtils.addToSelectedArr(meal, selectionArr, setSelectionArr)}></input>
			)
		}
  },
  favoriteIcon: (meal) => {
